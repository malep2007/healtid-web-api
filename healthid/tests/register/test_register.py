...
class RegisterTestCase(RegisterBaseCase):
     ...
     def test_empty_db(self):
         resp = self.query_with_token(self.access_token_master,'{register{id}}')
     ...
     def test_update_register(self):
         ...
         response = self.query_with_token(
           ...
           ''')) 
         self.assertResponseNoErrors(response, {"updateRegister": {"success": True, 'register': {'name': 'ever moore'}}})
     ...
     def test_register_model(self):
         ...
         self.assertQuerysetEqual(registers, [f'<Register: {register.name}>'])
